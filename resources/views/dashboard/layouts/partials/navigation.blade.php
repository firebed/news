<div id="sidebar" class="col-12 col-lg-3 col-xxl-2 px-0 sticky-top" x-data="{ show: false }">
    <div class="d-flex justify-content-between align-items-center sticky-top px-3 w-100" style="height: 3.5rem; background-color: rgb(49, 58, 70);">
        <div class="fs-5 fw-500 text-light d-flex justify-content-between">
            <div class="bg-red-500 rounded text-center me-2" style="width: 2rem">{{ \Illuminate\Support\Str::of(config('app.name'))->ucfirst()->substr(0, 1) }}</div>
    ...
    <div x-bind:class="{ 'show': show }" class="sidebar sticky-lg-top w-100" data-bs-backdrop="false" style="--top: 3.5rem">
        <div class="h-100" data-simplebar>
            <div class="d-grid">
                ...
                @can('View articles')
                    <a href="{{ route('admin.articles.index') }}" class=""><i class="fas fa-align-left w-2r"></i>{{ __("All articles") }}</a>
                @endcan

                @can('Create article')
                    <a href="{{ route('admin.articles.create') }}" class=""><i class="fas fa-plus w-2r"></i>{{ __("Add article") }}</a>
                @endcan
                ...
                @can('View users')
                    <a href="{{ route('admin.users.index') }}" class=""><i class="fas fa-users w-2r"></i>{{ __("All users") }}</a>
                @endcan

                @can('Create user')
                    <a href="{{ route('admin.users.create') }}" class=""><i class="fas fa-user-plus w-2r"></i>{{ __("Add user") }}</a>
                @endcan
            </div>
        </div>
    </div>
