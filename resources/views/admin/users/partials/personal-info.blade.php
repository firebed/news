    ...
    <div class="card-body">
        ...
        <div class="row mb-3">
            <div class="col">
                ...
                <x-input.text wire:model="user.first_name" id="first-name" error="user.first_name"/>
            </div>
            <div class="col">
                <label for="last-name" class="form-label small text-secondary">{{ __("Surname") }}</label>
                <x-input.text wire:model="user.last_name" id="first-name" error="user.last_name"/>
            </div>
        </div>
        <div class="row mb-3">
            <div class="col">
                <label for="title" class="form-label small text-secondary">{{ __("validation.attributes.title") }}</label>
                <x-input.text wire:model.defer="user.title" id="title" error="user.title"/>
            </div>
            <div class="col">
                <label for="slug" class="form-label small text-secondary">{{ __("Slug") }}</label>
                <x-input.text wire:model.defer="user.slug" id="slug" error="user.slug"/>
            </div>
        </div>
        <div class="row">
            <div class="col">
                <label for="email" class="form-label small text-secondary">{{ __("Email") }}</label>
                <x-input.email wire:model.defer="user.email" id="email" error="user.email"/>
            </div>
            <div class="col">
                <label for="password" class="form-label small text-secondary">{{ __("Password") }}</label>
                <x-input.text wire:model.defer="password" id="password" error="password"/>
            </div>
        </div>
    </div>
</div>
