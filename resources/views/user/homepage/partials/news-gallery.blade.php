<x-carousel class="pb-sm-3 carousel-fade" id="gallery">
    <x-carousel.inner>
        @foreach($gallery_news as $article)
            <x-carousel.item active="{{ $loop->first }}" :interval="$loop->first ? 8000 : NULL">
                ...
                <x-carousel.caption class="d-none d-sm-block start-0 bottom-0 w-100 text-start gallery-caption">
                    <div class="mb-3">
    ...
    <x-carousel.controls target="#gallery"/>

    <x-carousel.indicators class="mx-0" style="bottom: -19px">
        @foreach($gallery_news as $article)
            <x-carousel.indicator class="pt-1 wpx-4 border-0" target="#gallery" slide="{{ $loop->index }}" :active="$loop->first" style="border-bottom: 7px solid #212529 !important; margin: 0 1px !important"/>
        @endforeach
