...
{{-- For mobiles --}}
<x-news::carousel class="pb-sm-3 d-sm-none" id="columns">
    <x-news::carousel.inner>
        @foreach($columns as $article)
            <x-news::carousel.item active="{{ $loop->first }}">
                <a href="{{ route('user.articles.show', [$article->type->slug, $article->slug]) }}">
                    <div class="ratio ratio-16x9">
                        @if($article->image)
                ...
                <x-news::carousel.caption class="d-none d-sm-flex start-0 bottom-0 w-100 text-start gallery-caption">
